"""
build_Таблиця_5_1.py
Генерує Таблиця_5_1.xlsx — міжпостові коефіцієнти кореляції PM10 за місяцями
(pan2, pan6, pan5) — Таблиця 5.1 Розділу 5 дисертації
"""
import openpyxl
...
OUT = Path(__file__).parent / "Таблиця_5_1.xlsx"
...
ws = wb.active
ws.title = "Таблиця 5.1"

ws.merge_cells('A1:E1')
ws['A1'].value = "Таблиця 5.1 – Узагальнення міжпостових коефіцієнтів кореляції PM₁₀ за місяцями"
ws['A1'].font = Font(bold=True, name='Times New Roman', size=12)
ws['A1'].alignment = Alignment(horizontal='center', wrap_text=True)
ws.row_dimensions[1].height = 30

for c, h in enumerate(['Місяць', 'r (pan2–pan6)', 'r (pan2–pan5)', 'r (pan6–pan5)', 'Примітка'], 1):
    hdr(ws, 2, c, h)
...
for r, (month, r1, r2, r3, note) in enumerate(DATA, 3):
    fill = "C6EFCE" if isinstance(r1, float) and r1 >= 0.80 else (
           "FFEB9C" if isinstance(r1, float) and r1 < 0.60 else None)
